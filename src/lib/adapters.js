...
function resolveHomeOverride(value, home, fallbackName) {
  if (typeof value !== 'string' || !value.trim()) return path.join(home, fallbackName);
...
function createAdapters(home = os.homedir(), env = process.env) {
  ...
  const dshHome = resolveHomeOverride(env.DSH_HOME, home, '.dsh');
  ...
  return [
    ...
    },
    {
      ...
      detectPath: dshHome,
      command: 'dsh',
      fileLabel: '用户全局 AGENTS.md',
      scope: '所有 DeepSeek Harness workspace',
      note: '写入 $DSH_HOME/AGENTS.md；DeepSeek Harness 会在项目指令之前加载这份用户全局指令。'
    },
    {
      id: 'hermes',
      name: 'Hermes',
